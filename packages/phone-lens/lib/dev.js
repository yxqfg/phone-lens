...
async function startLensServer(deps) {
	...
	const uploadLimiter = new RateLimiter(6e4, config$1.limits.uploadsPerMinute);
...
async function handle$1(deps, req, res, ctx) {
	...
	if (method === "POST" && path === "/upload") {
		...
		log$1("info", `stored ${name} (${buf.byteLength}B) 鈫?${admitted.storage}:${admitted.ref.attachmentId}`);
		...
		if (folderMode) {
			...
			try {
				...
				log$1("info", `folder-mode saved: ${join(dir, name)}`);
			} catch (error) {
				log$1("warn", `folder save failed (${String(error)}) 鈥?falling back to composer staging`);
			}
		}
