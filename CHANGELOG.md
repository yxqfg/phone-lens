...
> 注：0.3.3/0.3.4 曾短暂发布后从 npm 移除（投递模式设计回退），故本版直接从 0.3.2 跨越。

### Changed
- **回退投递模式（v0.3.3/v0.3.4 的全部改动）**：移除「对话/文件夹/两者」投递模式、
  目录索引自动/手动通知、相关端点与配置——照片恢复为只放入对话框草稿
- 保留 0.3.2 的全部稳定能力（二维码自动刷新、加载修复、依赖根治）与
  适配 DSH 0.1.5 的 peer 声明修正
